...
#         # Fetch all existing record IDs
#         url = f"{DATAVERSE_BASE}{DATAVERSE_API}/{ENTITY_SET_contributors}?$select=crc6f_recordid&$top=5000"
#         res = requests.get(url, headers=hdr, timeout=20)

#         existing = set()
#         if res.ok:
#             for r in res.json().get("value", []):
#                 rid = r.get("crc6f_recordid")
#                 if rid:
#                     existing.add(rid)

#         # Find next unique REC###
#         num = 1
#         while True:
#             new_id = f"REC{num:03d}"
#             if new_id not in existing:
#                 return new_id
#             num += 1

#     except Exception as e:
#         print(f"⚠ Error generating record_id: {e}")
#         return "REC001"
...
#     try:
#         # --- 1) Count contributors for project_code using $count=true ---
#         # Don't use $top=0 (Dataverse rejects that). Use $count=true with $top=1 to get @odata.count.
#         count_url = (
#             f"{DATAVERSE_BASE}{DATAVERSE_API}/{ENTITY_SET_contributors}"
#             f"?$filter=crc6f_projectid eq '{project_code}'&$count=true&$top=1"
#         )
#         cr = requests.get(count_url, headers=headers, timeout=20)
#         if not cr.ok:
...
#         cnt = 0
#         j = cr.json()
#         # Dataverse returns '@odata.count' when $count=true
#         if isinstance(j, dict) and '@odata.count' in j:
#             cnt = int(j.get('@odata.count', 0))
#         else:
#             # fallback: use length of value list (safe)
...
#         # --- 3) PATCH project header ---
#         update_url = f"{DATAVERSE_BASE}{DATAVERSE_API}/{PROJECT_HEADER_ES}({proj_rec_id})"
#         # Use integer if the column is numeric; some Dataverse instances accept numeric directly.
#         patch_body = {"crc6f_noofcontributors": str(cnt)}
#         rpatch = requests.patch(update_url, headers={**headers, "Content-Type": "application/json"}, json=patch_body, timeout=20)
#         if not rpatch.ok:
...
# #         # Log everything (all projects) to terminal
# #         current_app.logger.info("=== ALL CONTRIBUTORS IN DATAVERSE ===")
# #         for c in data.get("value", []):
# #             current_app.logger.info({
# #                 "record_id": c.get(F_RECORD_ID),
# #                 "employee_id": c.get(F_EMP_ID),
# #                 "project_id": c.get(F_PROJECT_ID)
# #             })
# #         current_app.logger.info("=== END OF LIST ===")

# #         # Filter only this project's contributors for frontend
# #         contributors = []
...
#         # 3️⃣ Fetch employee master data once
#         token = get_access_token()
#         emp_entity = "crc6f_table12s"  # your Employee Master table
...
#         # 4️⃣ Attach designation from Employee Master
...
#         # ✅ Prepare payload for Dataverse (no max restriction)
#         dv_payload = {
#             "crc6f_employeeid": body.get("employeeId"),
...
#             "crc6f_recordid": generated_recordid,  # include our readable ID
#         }
...
#         res = requests.post(create_url, headers=headers, json=dv_payload, timeout=20)
#         current_app.logger.info("Dataverse create response: %s", res.status_code)

#         if res.status_code in (200, 201, 204):
#             # ✅ Auto recount after create
#             try:
...
#             return jsonify({"success": True, "message": "Contributor added"}), 201
#         else:
...
#         current_app.logger.info(f"✏️ Updating contributor {guid}")
...
#         # 🟢 Use the Dataverse GUID directly here
...
#         token = get_access_token()
#         headers = {"Authorization": f"Bearer {token}", "Accept": "application/json"}
#         current_app.logger.info("🗑️ Delete contributor called for GUID %s", guid)

#         # 1️⃣ Fetch the contributor (to get project_id for recount)
#         get_url = f"{DATAVERSE_BASE}{DATAVERSE_API}/{ENTITY_SET_contributors}({guid})?$select=crc6f_projectid"
#         gres = requests.get(get_url, headers=headers, timeout=20)
#         project_id = None
#         if gres.ok:
