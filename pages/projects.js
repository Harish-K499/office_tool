...
// -----------------------------------------------
// CRM TAB (Dynamic Columns + Default Columns)
// -----------------------------------------------
const crmTab = async (p, canManage = null) => {
  const allowManage =
    typeof canManage === "boolean" ? canManage : getProjectAccess().canManage;
  const projectId = p.crc6f_projectid || p.id;

  // Read selected board
  const urlParams = new URLSearchParams(
    window.location.hash.split("?")[1] || ""
  );
  const selectedBoardCode = decodeURIComponent(
    urlParams.get("board") || ""
  ).trim();
  const selectedBoardName = decodeURIComponent(
    urlParams.get("boardName") || ""
  ).trim();
  // Keep existing variable name for rest of function (represents board code)
  let selectedBoard = selectedBoardCode;

  // -----------------------------------------------
  // 1️⃣ Fetch Columns for This Board
  // -----------------------------------------------
  let columns = [];
  try {
    const colRes = await fetch(
      `http://localhost:5000/api/projects/${projectId}/columns?board=${encodeURIComponent(
        selectedBoard
      )}`
    );
    ...
    <div class="kan-col" data-col="${c}" style="background:${bg};">
      <div class="kan-head">
        <div style="display:flex; align-items:center; gap:8px;">
          <h4 style="margin:0; font-size:14px;">${c}</h4>
          <span class="badge low">${(byCol[c] || []).length}</span>
        </div>
        ${menuHtml}
      </div>
      <div class="kan-list" data-col="${c}">
        ${(byCol[c] || []).map(taskCardHtml).join("")}
      </div>
    </div>
  `;
  };
  ...
  return `
    ...
    <div class="kan-wrap">
      ${window.GLOBAL_CRM_COLS.map((c, idx) => colHtml(c, idx)).join("")}


      <!-- ADD COLUMN BUTTON -->
      ${allowManage
      ? `<div class="kan-col add-col-box" onclick="openAddColumnModal('${projectId}', '${selectedBoard}')">
