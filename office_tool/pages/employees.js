...
export const renderEmployeesPage = async (filter = '', page = empCurrentPage) => {
    ...
    // Initialize state.employees if not already set
    if (!state.employees) {
        state.employees = [];
    }
    ...
    try {
        ...
        console.log('[EMPLOYEES_PAGE] Loaded', state.employees.length, 'employees');
        ...
        console.error('[EMPLOYEES_PAGE] Failed to load employees from backend:', err);
    ...
    const errorBanner = errorMessage ? `
        <div class="alert alert-danger" style="margin-bottom: 1rem;">
            ...
            <strong>Error loading employees:</strong> ${errorMessage}
        </div>
    ` : '';
