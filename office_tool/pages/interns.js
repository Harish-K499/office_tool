...
import { listInterns, createIntern, deleteIntern } from '../features/internApi.js';
...
const attachInternEvents = () => {
  ...
  document.querySelectorAll('.intern-delete-btn').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const internId = btn.getAttribute('data-intern-id');
      if (!internId) return;
      
      if (confirm(`Are you sure you want to delete intern ${internId}? This action cannot be undone.`)) {
        try {
          btn.disabled = true;
          btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
          await deleteIntern(internId);
          alert('Intern deleted successfully');
          renderInternsPage(internSearch, internPage);
        } catch (err) {
    ...
    <div class="employee-card">
      <div class="employee-card-header">
        ...
        <div class="employee-card-actions">
          <button class="icon-btn intern-delete-btn" data-intern-id="${intern.intern_id}" title="Delete Intern"><i class="fa-solid fa-trash"></i></button>
